...
const luckyuser = document.getElementById('luckyperson').addEventListener('click',function(){
  socket.emit('luckyperson');
});

socket.on('luckypersonfound', (user) => {
  // Scroll down 
  botluckymsg(user);
  chatMessages.scrollTop = chatMessages.scrollHeight;
})

function botluckymsg(user) {
  const div = document.createElement('div');
  div.classList.add('botwarn');
  div.innerHTML = `
  <p >
    CHAT ROOM BOT :  🥳 Congratulation ${user} you are Selected 🎊.
  </p>`;
  document.querySelector('.chat-messages').appendChild(div);
}
